...
// ─── Adjust GST Panel ────────────────────────────────────────────────────────
const AdjustGSTPanel = ({ onClose, entitiesList }) => {
  const [form, setForm] = useState({
    entity: "",
    adjustment_month: new Date().toISOString().slice(0, 7),
    amount: "",
    adjustment_type: "",
    remarks: "",
  });
  ...
  const validate = () => {
    ...
    if (!form.adjustment_month) e.adjustment_month = "Month is required";
    if (!form.amount || parseFloat(form.amount) <= 0)
      e.amount = "Valid amount is required";
    if (!form.adjustment_type) e.adjustment_type = "Select Add or Subtract";
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await supabase.from("gst_adjustments").insert([
        {
          entity: form.entity,
          adjustment_month: `${form.adjustment_month}-01`,
          adjustment_type: form.adjustment_type,
          amount: parseFloat(form.amount),
          remarks: form.remarks || null,
          ops_percentage: 0,
          temp_percentage: 0,
          recruitment_percentage: 0,
          projects_percentage: 0,
          others_percentage: 100,
        },
      ]);
      if (error) throw error;
      window.refreshDashboard?.();
      showToast("GST Adjustment saved successfully");
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-black">
                {viewMode ? "GST Adjustments" : "Adjust GST"}
              </p>
              <p className="text-[11px] text-cyan-200 mt-0.5">
                {viewMode
                  ? "View & delete manual GST entries"
                  : "Manual GST correction by entity & month"}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold border border-white/20 transition-all"
            >
              {viewMode ? (
                <>
                  <Pencil className="w-3.5 h-3.5" /> Add New
                </>
              ) : (
                <>
                  <Eye className="w-3.5 h-3.5" /> View Entries
                </>
              )}
            </button>
            <button
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50/30">
        {viewMode ? (
          ...
          <div className="space-y-4">
            ...
            {loadingAdj ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-300">
                ...
                <p className="text-sm font-semibold">No adjustments found</p>
              </div>
            ) : (
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-500 mb-2">
                          <span>
                            ...
                            <span className="font-semibold text-gray-700">
                              {new Date(adj.adjustment_month).toLocaleDateString(
                                "en-IN",
                        ...
                        {deletingId === adj.id ? (
                          ...
                          <div className="flex flex-col gap-1.5">
                            <button
                              ...
                              className="px-3 py-1.5 bg-rose-500 text-white text-[10px] font-black rounded-xl hover:bg-rose-600 transition-colors"
                            >
                              Confirm
                            </button>
                            <button
                              onClick={() => setConfirmId(null)}
                              className="px-3 py-1.5 border-2 border-gray-100 text-gray-400 text-[10px] font-bold rounded-xl hover:bg-gray-50 transition-colors"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="p-2 rounded-xl border-2 border-transparent text-gray-300 hover:text-rose-500 hover:bg-rose-50 hover:border-rose-100 transition-all"
                          >
          ...
          /* ═══════════════════════ EXISTING FORM ═══════════════════════ */
          <>
            <Section icon={IndianRupee} title="Adjustment Details" color="blue">
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <input
                    type="month"
                    value={form.adjustment_month}
                    onChange={(e) =>
                      handleChange("adjustment_month", e.target.value)
                    }
                    className={inpCls(showErrors && errors.adjustment_month)}
                  />
                </Field>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <Field
                  ...
                  error={errors.amount}
                  showErrors={showErrors}
                  hint="Positive value only"
                >
                  <input
                    type="number"
                    value={form.amount}
                    onChange={(e) => handleChange("amount", e.target.value)}
                    className={inpCls(showErrors && errors.amount)}
                    placeholder="₹ 0"
                  />
                </Field>
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => handleChange("adjustment_type", "add")}
                      className={`flex items-center justify-center gap-1.5 py-3 rounded-xl font-black text-sm transition-all border-2 ${
                    ...
                    <button
                      type="button"
                      onClick={() => handleChange("adjustment_type", "subtract")}
                      className={`flex items-center justify-center gap-1.5 py-3 rounded-xl font-black text-sm transition-all border-2 ${
                        form.adjustment_type === "subtract"
                          ? "bg-rose-500 text-white border-rose-500 shadow-lg shadow-rose-500/20"
                          : "bg-white text-gray-400 border-gray-100 hover:border-gray-200"
                      }`}
                    >
                      <TrendingDown className="w-3.5 h-3.5" /> Subtract
                    </button>
                  </div>
                </Field>
              </div>
              ...
              <div className="mt-3 flex items-center gap-2 text-[11px] text-gray-500">
                ...
                <span>
                  <span className="font-bold text-emerald-600">Add</span> increases
                  GST due for the month.{" "}
                  <span className="font-bold text-rose-600">Subtract</span> reduces
                  it (e.g. credit notes, corrections).
                </span>
              </div>
            </Section>
            ...
            <div className="flex items-center justify-between pt-2 pb-1">
              ...
              >
                {loading ? (
                  ...
                  <>
                    Save Adjustment <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
            </div>
...
const AddStatutoryPayoutModal = ({
  ...
  const [adjustOpen, setAdjustOpen] = useState(false); // ✅ Step 2A
            ...
            <div className="bg-gradient-to-r from-cyan-600 via-cyan-500 to-blue-600 px-7 py-5 text-white flex-shrink-0">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <IndianRupee className="w-3.5 h-3.5" /> Adjust GST
                  </button>
                  <button
                    type="button"
