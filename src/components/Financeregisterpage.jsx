...
const TYPE_CONFIG = {
  "Payment Received": { icon: ArrowDownLeft, color: "#059669", bg: "#ecfdf5", border: "#a7f3d0" },
  "Payment Made": { icon: ArrowUpRight, color: "#dc2626", bg: "#fef2f2", border: "#fecaca" },
  "Employee Payout": { icon: Users, color: "#0891b2", bg: "#ecfeff", border: "#a5f3fc" },
  "OS Payout": { icon: Banknote, color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe" },
  Expense: { icon: Receipt, color: "#d97706", bg: "#fffbeb", border: "#fde68a" },
  "Statutory Payment": { icon: ShieldCheck, color: "#0d9488", bg: "#f0fdfa", border: "#99f6e4" },
  "Advance Payment": { icon: Wallet, color: "#4f46e5", bg: "#eef2ff", border: "#c7d2fe" },
  "Client Advance Tracker": { icon: Landmark, color: "#4338ca", bg: "#eef2ff", border: "#c7d2fe" },
  "Bounce Back": { icon: Undo2, color: "#c026d3", bg: "#fdf4ff", border: "#f5d0fe" },
};

// Fallback config for dynamic types like "CN / Bad Debt - CN" and "Interest/Penalty - Late Fee"
function getTypeConfig(type) {
  if (TYPE_CONFIG[type]) return TYPE_CONFIG[type];
  if (type?.startsWith("CN / Bad Debt")) return { icon: FileMinus2, color: "#ea580c", bg: "#fff7ed", border: "#fed7aa" };
  if (type?.startsWith("Interest/Penalty")) return { icon: AlertTriangle, color: "#b91c1c", bg: "#fef2f2", border: "#fecaca" };
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="truncate max-w-[140px]">{value ? selectedOption?.label || value : label}</span>
...
function TransactionRow({ tx, index }) {
  ...
  return (
    ...
    >
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{formatDate(tx.payment_date)}</td>
      ...
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{tx.department || "—"}</td>
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{tx.entity || "—"}</td>
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{formatMonth(tx.payout_month)}</td>
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap text-center">
        {tx.head_count ?? "—"}
      </td>
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{tx.bank_name || "—"}</td>
      <td className="px-3 py-3 text-xs max-w-[180px]">
        <span className="text-gray-400 truncate block">{tx.remarks || "—"}</span>
      </td>
...
export default function FinanceRegisterPage() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      ...
      if (data) {
        const types = [...new Set(data.map((d) => d.transaction_type).filter(Boolean))].sort();
        const entities = [...new Set(data.map((d) => d.entity).filter(Boolean))].sort();
        const departments = [...new Set(data.map((d) => d.department).filter(Boolean))].sort();
        setTypeOptions(types.map((t) => ({ value: t, label: t, icon: getTypeConfig(t).icon, color: getTypeConfig(t).color })));
  ...
  const buildQuery = useCallback(() => {
    let q = supabase.from("finance_transaction_register_view").select("*", { count: "exact" });
  ...
  const fetchRows = useCallback(
    async (isAuto = false) => {
      ...
      try {
        ...
        setRows(data || []);
        setRows((prev) => {
          // store count separately via state below
          return data || [];
        });
        setTotalCountRef(count || 0);

        // fetch all matching rows (amount only) for totals
        const totalsQ = buildQuery().select("amount, transaction_type");
  ...
  const setTotalCountRef = (n) => setTotalCountState(n);
  ...
  // Totals
  const grandTotal = allRowsForTotals.reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
  const inflowTypes = ["Payment Received", "Advance Payment"];
  const outflowTypes = [
    "Payment Made",
    "Employee Payout",
    "OS Payout",
  ...
  const inflowTotal = allRowsForTotals
    .filter((r) => inflowTypes.includes(r.transaction_type))
    .reduce((sum, r) => sum + (Number(r.amount) || 0), 0);
  const outflowTotal = allRowsForTotals
    .filter((r) => outflowTypes.some((t) => r.transaction_type?.startsWith(t)) || r.transaction_type?.startsWith("Interest/Penalty"))
    .reduce((sum, r) => sum + (Number(r.amount) || 0), 0);

  const handleExport = () => {
    if (!rows.length) return;
    const headers = [
      "Date", "Type", "Client/Employee", "Emp Code", "Employee Email", "Invoice/Ref", "Department",
      "Entity", "Payout Month", "Head Count", "Pay Head", "Bank", "Remarks", "Status", "Created By", "Amount",
    ];
    ...
    a.href = url;
    a.download = `finance-register-${formatDateInput(new Date())}.csv`;
    a.click();
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900 tracking-tight">Finance Transaction Register</h2>
            <p className="text-xs text-gray-400">
              {totalCount.toLocaleString("en-IN")} transaction{totalCount === 1 ? "" : "s"}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(16,185,129,0.10)" }}>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center">
              <ArrowDownLeft className="w-4 h-4 text-emerald-600" />
            </div>
            <p className="text-xs font-semibold text-gray-500">Total Inflow</p>
          </div>
          <p className="text-2xl font-bold text-emerald-600">{formatAmount(inflowTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">Payments Received + Advances</p>
        </div>

        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(220,38,38,0.10)" }}>
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-2xl font-bold text-rose-600">{formatAmount(outflowTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">Payments, Payouts, Expenses, Statutory</p>
        </div>

        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(59,130,246,0.10)" }}>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center">
              <IndianRupee className="w-4 h-4 text-blue-600" />
            </div>
            <p className="text-xs font-semibold text-gray-500">All Transactions (filtered)</p>
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatAmount(grandTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">{allRowsForTotals.length.toLocaleString("en-IN")} entries</p>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <FilterDropdown label="All types" icon={Activity} value={type} onChange={setType} options={typeOptions} allLabel="All types" />
          <FilterDropdown label="All entities" icon={Building2} value={entity} onChange={setEntity} options={entityOptions} allLabel="All entities" />
          <FilterDropdown label="All departments" icon={Briefcase} value={department} onChange={setDepartment} options={departmentOptions} allLabel="All departments" />
          ...
          <div className="relative">
            ...
            >
              ...
              {customRange ? `${customRange.from} → ${customRange.to}` : DATE_PRESETS.find((p) => p.id === datePreset)?.label}
      ...
      {/* ── Table ── */}
      <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(59,130,246,0.08)" }}>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-semibold text-rose-700">Couldn't load transactions</p>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1 max-w-sm">
              {activeFilterCount > 0 || search
                ...
                : "Transactions will appear here as they're recorded across the system."}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[1300px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[
                    "Date", "Type", "Client / Employee", "Invoice / Ref", "Department", "Entity",
                    "Payout Month", "Head Count", "Bank", "Remarks", "Status", "Created By",
                  ].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left whitespace-nowrap">
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {rows.map((tx, idx) => (
                  <TransactionRow key={`${tx.transaction_type}-${tx.id}`} tx={tx} index={idx} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      {!loading && totalCount > PAGE_SIZE && (
        <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(0)} disabled={page === 0} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
              <ChevronsLeft className="w-4 h-4" />
            </button>
            <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="px-3 text-xs font-semibold text-gray-600">
              Page {page + 1} of {totalPages}
            </span>
            <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
              <ChevronRight className="w-4 h-4" />
            </button>
            <button onClick={() => setPage(totalPages - 1)} disabled={page >= totalPages - 1} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
