...
// ─── Safe RPC wrapper (prevents one broken function from crashing the page) ─────
...
const FYSelector = ({
  ...
  return (
    <div className="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-full px-1.5 py-1">
      ...
      <span className="text-xs font-bold text-gray-700 px-2 tracking-wide whitespace-nowrap">
        {label}
      </span>
      <button
...
export default function AnalyticsDashboard() {
    ...
    try {
      const [
        ...
        supabase
          .from("internal_team")
          .select(
            "id,name,emp_code,department,designation,location,ctc,status,doj,entity"
          ),
        supabase
          .from("bank_entries")
  ...
  const entityById = useMemo(() => {
    const m = {};
    entities.forEach((e) => {
      m[e.id] = e.entity_name;
    });
    return m;
  }, [entities]);
  const empDeptByCode = useMemo(() => {
    const m = {};
    team.forEach((t) => {
      if (t.emp_code)
        m[t.emp_code] =
          TEAM_DEPT_MAP[t.department] || t.department || "Unknown";
    });
    return m;
  }, [team]);
  ...
  const FI = useMemo(
    () =>
      invoices.map((i) => ({
        ...
        entity_name:
          i.entity_master?.entity_name || entityById[i.entity_id] || "Unknown",
      })),
  ...
  const FO = useMemo(
    () =>
      osPayouts.map((o) => ({
        ...
        dept_name:
          deptById[o.department_id] ||
          deptById[o.invoices?.department_id] ||
          "Unknown",
        entity_name:
          o.entity_master?.entity_name || entityById[o.entity_id] || "Unknown",
  ...
  const FSal = useMemo(
    () =>
      salaries.map((s) => ({
        ...s,
        dept_name:
          s.departments_master?.dept_name ||
          empDeptByCode[s.emp_code] ||
          "Unknown",
        entity_name:
          s.entity_master?.entity_name || entityById[s.entity_id] || "Unknown",
      })),
  ...
  const fI = useMemo(
    () =>
      FI.filter((i) => {
        ...
        if (
          filters.impactMonth &&
          toYYYYMM(i.impact_month) !== filters.impactMonth
        )
          return false;
        if (filters.department && i.dept_name !== filters.department)
          return false;
  ...
  const fO = useMemo(
    () =>
      FO.filter((o) => {
        ...
        if (filters.department && o.dept_name !== filters.department)
          return false;
  ...
  const fC = useMemo(
    () =>
      FC.filter((cn) => {
        ...
        if (filters.department && cn.dept_name !== filters.department)
          return false;
  ...
  const fSal = useMemo(
    () =>
      FSal.filter((s) => {
        ...
        if (filters.department && s.dept_name !== filters.department)
          return false;
  ...
  const fTeam = useMemo(
    () =>
      team.filter((t) => {
        ...
        if (
          filters.employee &&
          !t.name?.toLowerCase().includes(filters.employee.toLowerCase())
        )
          return false;
        return true;
      }),
  ...
  const kpis = useMemo(() => {
    ...
    const totalOut = fI.reduce(
      (s, i) => s + Number(i.receivable_amount || 0),
      0
    );
    ...
    const colPct =
      totalInv > 0 ? ((totalRcv / totalInv) * 100).toFixed(1) : "0.0";
    const bIn = fBank
      .filter(isBankInflow)
      .reduce((s, b) => s + Math.abs(Number(b.amount || 0)), 0);
    const bOut = fBank
      .filter((b) => !isBankInflow(b))
      .reduce((s, b) => s + Math.abs(Number(b.amount || 0)), 0);
    return {
      totalInv,
      netV,
    ...
    return Object.values(m)
      ...
      .map((d) => ({
        ...
        netVerto:
          ...
          fC.reduce(
            (s, c) =>
              toYYYYMM(c.issue_date) === d.month
                ? s + Number(c.verto_fee_cn || 0)
                : s,
            0
          ),
      }));
  ...
  const revByClientAll = useMemo(() => {
    ...
    return Object.values(m).sort(
      (a, b) => b["Invoice Value"] - a["Invoice Value"]
    );
  }, [fI, fP, fC]);

  const revByClient = useMemo(
    () =>
      topNClient === 999 ? revByClientAll : revByClientAll.slice(0, topNClient),
    [revByClientAll, topNClient]
  );
  ...
  const revByDept = useMemo(() => {
    ...
    fI.forEach((i) => {
      const d = i.dept_name;
      if (!m[d])
        m[d] = { dept: d, invoiceValue: 0, vertoFee: 0, cnDeducted: 0 };
  ...
  const collAll = useMemo(() => {
    ...
    fP.forEach((p) => {
      if (p.invoice_number)
        rcvByInv[p.invoice_number] =
          (rcvByInv[p.invoice_number] || 0) + Number(p.amount_received || 0);
    });
    return fI
      .map((i) => {
        const rcv =
          rcvByInv[i.invoice_number] ?? Number(i.amount_received || 0);
        const pct =
          Number(i.invoice_value) > 0
  ...
  const coll = useMemo(
    () => (topNInvoice === 999 ? collAll : collAll.slice(0, topNInvoice)),
    [collAll, topNInvoice]
  );
  ...
  const osBillable = useMemo(() => {
    ...
    return [
      { name: "Billable", value: b },
      { name: "Non-Billable", value: nb },
    ].filter((d) => d.value > 0);
  }, [fO]);
  ...
  const salByDept = useMemo(
    () =>
      topNSalary === 999 ? salByDeptAll : salByDeptAll.slice(0, topNSalary),
    [salByDeptAll, topNSalary]
  );
  ...
  const teamByDept = useMemo(
    () => (topNTeam === 999 ? teamByDeptAll : teamByDeptAll.slice(0, topNTeam)),
    [teamByDeptAll, topNTeam]
  );
  ...
  const impMonthOpts = [
    ...new Set(invoices.map((i) => toYYYYMM(i.impact_month)).filter(Boolean)),
  ]
    .sort()
    .reverse()
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {AFC > 0 && (
              <span className="w-4 h-4 rounded-full bg-white text-slate-700 text-[10px] font-black flex items-center justify-center">
                {AFC}
              </span>
            )}
          </button>
          <button
            onClick={fetchAll}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border border-gray-200 bg-white text-gray-600 hover:border-slate-300 transition-all"
          >
      ...
      {filtersOpen && (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[11px] text-gray-400">
                Date range defaults to{" "}
                <span className="font-semibold text-gray-600">{fy.label}</span>{" "}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <FS
              label="Impact Month"
              value={filters.impactMonth}
              onChange={(v) => setF("impactMonth", v)}
              options={impMonthOpts}
            />
            <FS
              label="Department"
            ...
            <FS
              label="Pay Head"
              value={filters.payHead}
              onChange={(v) => setF("payHead", v)}
              options={payHOpts}
            />
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setF("employee", e.target.value)}
                  placeholder="Search…"
                  className="w-full border border-gray-200 rounded-xl pl-8 pr-3 py-2 text-xs font-medium text-gray-700 focus:outline-none focus:border-slate-400"
                />
              </div>
        ...
        <KpiCard
          ...
          sub={
            outstandingBC.length > 0
              ? `${outstandingBC.length} clients pending`
              : "All cleared ✓"
          }
          icon={Activity}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ChartCard
          ...
          subtitle={`${revByClientAll.length} clients total`}
          topN={topNClient}
          onTopN={setTopNClient}
          topNOptions={[5, 10, 20, 999]}
          expandable
          onExpand={() =>
            setModal({
                  ...
                  {[
                    ...
                    <div key={k}>
                      ...
                      <VScrollBar
                        ...
                        nameKey="client"
                        color={c}
                        height={Math.min(600, revByClientAll.length * 40 + 20)}
                      />
                    </div>
                  ))}
                </div>
        ...
        <ChartCard
          title="Pay Head Distribution"
          subtitle="Invoice value by pay head"
        >
          {payHeadD.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          title="GST & TDS by Month (Net after CN)"
          subtitle="Scroll for many months"
          scrollable
          minScrollWidth={Math.max(400, invByMonth.length * 80)}
        >
          <HScrollBar
            data={invByMonth}
            xKey="month"
      ...
      {(fC.length > 0 || creditNotes.length > 0) && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard
              title="CN Amount by Month"
              subtitle="Total credit note value raised"
            >
              {cnByMonth.length === 0 ? (
                <Empty msg="No credit notes in selected range" />
              ) : (
                <HScrollBar
                  data={cnByMonth}
                  xKey="month"
            ...
            <ChartCard
              title="CN by Client"
              subtitle="Credit note impact per client"
            >
              {cnByClient.length === 0 ? (
                <Empty msg="No credit notes in selected range" />
              ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard
          ...
          subtitle={`${collAll.length} invoices`}
          topN={topNInvoice}
          onTopN={setTopNInvoice}
          topNOptions={[5, 10, 20, 999]}
          scrollable
          minScrollWidth={Math.max(400, coll.length * 100)}
        >
          <HScrollBar
            data={coll}
            xKey="invoice"
        ...
        <ChartCard
          ...
          subtitle="% of invoice value collected"
          topN={topNInvoice}
          onTopN={setTopNInvoice}
          topNOptions={[5, 10, 20, 999]}
          scrollable
          minScrollWidth={Math.max(400, coll.length * 80)}
        >
          <div className="overflow-x-auto pb-1">
              ...
              <ResponsiveContainer width="100%" height={220}>
                ...
                >
                  ...
                  <XAxis
                    dataKey="invoice"
                    tick={{ fontSize: 9 }}
                    angle={coll.length > 6 ? -20 : 0}
                    textAnchor={coll.length > 6 ? "end" : "middle"}
                  />
                  <YAxis
                    tick={{ fontSize: 10 }}
                    tickFormatter={(v) => `${v}%`}
        ...
        <ChartCard
          ...
          subtitle="Remaining receivable"
          expandable
          onExpand={() =>
            setModal({
              title: "Outstanding by Client",
              subtitle: `${outstandingBC.length} clients`,
              content: (
                <VScrollBar
                  data={outstandingBC}
                  dataKey="outstanding"
                  nameKey="client"
                  color={P.amber}
                  height={Math.min(700, outstandingBC.length * 44 + 20)}
                />
              ),
            })
          }
        >
          {outstandingBC.length === 0 ? (
            ...
            <VScrollBar
              ...
              nameKey="client"
              color={P.amber}
              height={Math.min(280, outstandingBC.length * 44 + 20)}
            />
          )}
        </ChartCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard
          title="OS Payout by Month"
          subtitle="Amount paid per month (uses payment_date)"
          scrollable
          minScrollWidth={Math.max(400, osByMonth.length * 80)}
        >
          {osByMonth.length === 0 ? (
        ...
        <ChartCard
          title="OS Payout by Client"
          subtitle={`${osByClientAll.length} clients`}
        >
          {osByClientAll.length === 0 ? (
            <Empty />
          ) : (
            <VScrollBar
              data={osByClientAll}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard
          title="Salary Paid by Month"
          subtitle="Net salary outflow with brush to zoom"
        >
          {salByMonth.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          ...
          subtitle="Falls back to internal_team dept via emp_code"
          topN={topNSalary}
          onTopN={setTopNSalary}
          topNOptions={[5, 10, 999]}
        >
          {salByDept.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          title="Salary Pay Head Split"
          subtitle="Distribution by pay head"
        >
          {salPayHead.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          ...
          subtitle="By net_payment across filtered salary entries"
        >
          {fSal.length === 0 ? (
            <Empty />
          ) : (
            (() => {
              const top = [...fSal]
                .sort(
                  (a, b) =>
                    Number(b.net_payment || 0) - Number(a.net_payment || 0)
                )
                .slice(0, 10);
              return (
                <div className="overflow-auto max-h-[260px]">
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-white border-b border-gray-100">
                      <tr>
                        <th className="text-left py-2 pr-3 text-gray-400 font-semibold">
                          Employee
                        </th>
                        <th className="text-left py-2 pr-3 text-gray-400 font-semibold">
                          Dept
                        </th>
                        <th className="text-right py-2 text-gray-400 font-semibold">
                          Net Pay
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {top.map((s, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="py-2 pr-3 font-medium text-gray-800 truncate max-w-[130px]">
                            {s.employee_name || "—"}
                          </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          {teamByDept.length === 0 ? (
            ...
            <ResponsiveContainer
              width="100%"
              height={Math.min(280, teamByDept.length * 52 + 20)}
            >
              <BarChart
                data={teamByDept}
                layout="vertical"
        ...
        <ChartCard
          title="Total CTC by Department"
          subtitle="Monthly salary burden"
        >
          {teamByDept.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          ...
          subtitle={`${locationDist.length} cities`}
          expandable
          onExpand={() =>
            setModal({
        ...
        <ChartCard
          ...
          subtitle={`${designations.length} unique designations`}
          expandable
          onExpand={() =>
            setModal({
        ...
        >
          {(() => {
            const m = {};
            [...fTeam]
              .sort((a, b) => (a.doj || "").localeCompare(b.doj || ""))
              .forEach((t) => {
                const k = toYYYYMM(t.doj);
                if (!k) return;
                m[k] = (m[k] || 0) + 1;
              });
            let running = 0;
            const data = Object.entries(m)
              ...
              .map(([k, v]) => {
                running += v;
                return { month: fmtMonth(k), joined: v, total: running };
              });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard
          title="Bank Inflow vs Outflow"
          subtitle="Scroll/brush to navigate dates"
        >
          {bankFlow.length === 0 ? (
            <Empty />
          ) : (
        ...
        <ChartCard
          title="Software Balance Flow"
          subtitle="Scroll/brush to navigate"
        >
          {swFlow.length === 0 ? (
            <Empty msg="No software entry data" />
          ) : (
            <BrushArea
              data={swFlow}
              xKey="date"
        ...
        <ChartCard
          ...
          subtitle="All entries grouped by transaction type"
          className="lg:col-span-2"
        >
          {cashBySource.length === 0 ? (
            <Empty />
          ) : (
      ...
      {statByInv.length > 0 && (
        <>
          ...
          <ChartCard
            ...
            scrollable
            minScrollWidth={Math.max(500, statByInv.length * 100)}
          >
            <HScrollBar
              data={statByInv}
              xKey="invoice"
      ...
      {rpcProfit.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard
              title="Verto Fee vs Expense vs Profit (Monthly)"
              subtitle="Pre-TDS profit trend from profit_center_pl_view"
              scrollable
              minScrollWidth={Math.max(500, rpcProfit.length * 90)}
            >
              <HScrollBar
            ...
            <ChartCard
              ...
              subtitle="profit_pre_tds / verto_fee_earned × 100"
            >
              {rpcProfit.length === 0 ? (
                <Empty />
              ) : (
            ...
            <ChartCard
              title="Revenue vs Profit by Department"
              subtitle="From profit_center_pl_view — all months combined"
            >
              {rpcDeptRev.length === 0 ? (
                <Empty />
              ) : (
            ...
            <ChartCard
              title="Client P&L — Top 15"
              subtitle="From client_wise_pl_view · verto fee earned vs profit"
            >
      ...
      {(rpcFunnel.length > 0 || rpcAging.length > 0) && (
        <>
          <SH
            icon={DollarSign}
            title="Collection Funnel & Invoice Aging"
            color={P.teal}
          />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard
              title="Collection Funnel"
              subtitle="Invoiced → Expected → Collected → Outstanding"
            >
              {rpcFunnel.length === 0 ? (
                <Empty />
              ) : (
            ...
            <ChartCard
              title="Invoice Health Check"
              subtitle="Mismatches & delayed invoices"
            >
              {rpcInvHealth.length === 0 ? (
                <Empty />
              ) : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart
            ...
            <ChartCard
              title="Weekly Payment Collections"
              subtitle="Aggregated weekly — scale-safe for 1000+ invoices/month"
            >
              {rpcPayTrend.length === 0 ? (
                <Empty />
              ) : (
                <BrushArea
                  data={rpcPayTrend.map((r) => ({
                    week: r.week_start,
      ...
      {rpcHeadEcon.length > 0 && (
        <>
          <SH
            icon={Users}
            title="Headcount Economics — Revenue & Cost per Head"
            color={P.sky}
          />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard
      ...
      {rpcStatutory.length > 0 && (
        <>
          ...
          <ChartCard
            ...
            scrollable
            minScrollWidth={Math.max(500, rpcStatutory.length * 90)}
          >
            <HScrollBar
      ...
      {rpcBankWeekly.length > 0 && (
        <>
          <SH
            icon={Activity}
            title="Bank Flow — Weekly (Scale-Safe)"
            color={P.sky}
          />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartCard
              title="Weekly Bank Inflow vs Outflow"
              subtitle="1 year = 52 data points regardless of transaction volume"
            >
              <BrushArea
            ...
            <ChartCard
              title="Cash Flow by Source Type"
              subtitle="Pre-aggregated — safe at 100K+ bank entries"
            >
              {rpcBankSource.length === 0 ? (
                <Empty />
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart
      ...
      {rpcTopEarners.length > 0 && (
        <>
          ...
          <ChartCard
            ...
            subtitle="Aggregated from server — handles 7000+ payouts/month"
          >
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-white border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-right py-2 pr-3 text-gray-400 font-semibold">
                      Gross
                    </th>
                    <th className="text-right py-2 pr-3 text-gray-400 font-semibold">
                      TDS
                ...
                <tbody className="divide-y divide-gray-50">
                  {rpcTopEarners.map((r, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      <td className="py-2 pr-3 text-gray-300 font-bold">
                        {i + 1}
                      </td>
                      <td className="py-2 pr-3 font-semibold text-gray-800 truncate max-w-[140px]">
                        {r.employee_name || "—"}
                      </td>
                      <td className="py-2 pr-3 text-gray-400 truncate">
                        {r.dept_name}
                      </td>
                      ...
                      <td className="py-2 pr-3 text-right text-rose-400">
                        {fmt(r.total_tds)}
                      </td>
